...
def main(): 
	...
	data_gen_train = get_anchor_gt(train_imgs, classes_count, C, nn.get_img_output_length, keras.backend.image_data_format(), mode='train')
	data_gen_val = get_anchor_gt(val_imgs, classes_count, C, nn.get_img_output_length, keras.backend.image_data_format(), mode='val')

	print("TEST: ", data_gen_train)
	...
	model_rpn = keras.Model(img_input, rpn[:2])
	...
	optimizer_classifier = tf.keras.optimizers.Adam(learning_rate=1e-5)
	...
	for epoch_num in range(num_epochs):
		...
		while True:
			try:
				...
				tloss[iter_num, 0] = loss_rpn[1]
				tloss[iter_num, 1] = loss_rpn[2]

				tloss[iter_num, 2] = loss_class[1]
				tloss[iter_num, 3] = loss_class[2]
				tloss[iter_num, 4] = loss_class[3]

				#progbar.update(iter_num+1, [('rpn_cls', tloss[iter_num, 0]), ('rpn_regr', tloss[iter_num, 1]),
				#						  ('detector_cls', tloss[iter_num, 2]), ('detector_regr', tloss[iter_num, 3])])

				iter_num += 1
				
				if iter_num == epoch_length:
					loss_rpn_cls = np.mean(tloss[:, 0])
					loss_rpn_regr = np.mean(tloss[:, 1])
					loss_class_cls = np.mean(tloss[:, 2])
					loss_class_regr = np.mean(tloss[:, 3])
					class_acc = np.mean(tloss[:, 4])
